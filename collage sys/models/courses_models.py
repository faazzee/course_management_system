...
from database.sqlite_connections import create_db,create_engine
...
async def add(id,title,description):
    try:
        session=await create_db()
        query=insert(courses).values(id=id,title=title,description=description)
